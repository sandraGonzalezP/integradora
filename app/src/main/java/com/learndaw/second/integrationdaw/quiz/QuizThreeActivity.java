...
public class QuizThreeActivity extends AppCompatActivity {
    private QuestionsIIILibrary nQuestionIIILibrary = new QuestionsIIILibrary();
...
// This Line is opcional
                    Toast.makeText(QuizThreeActivity.this, "Correcto", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(QuizThreeActivity.this, "Error", Toast.LENGTH_SHORT).show();
                    updateQuestion();
                }
            }
        });
...
// Star of Button Listener for Button Quit
        nButtonQuit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intentLevel = new Intent(QuizThreeActivity.this, LevelsActivity.class);
    ...
    private void updateQuestion() {
        ...
        childUpdates.put("unit_three", nScore);
        myRef.updateChildren(childUpdates);
        if (nQuestionNumber == 10) {
            finishQuiz();
        } else {
            nQuestionView.setText(nQuestionIIILibrary.getQuestion(nQuestionNumber));
            nButtonChoice1.setText(nQuestionIIILibrary.getChoices1(nQuestionNumber));
            nButtonChoice2.setText(nQuestionIIILibrary.getChoices2(nQuestionNumber));
            nButtonChoice3.setText(nQuestionIIILibrary.getChoices3(nQuestionNumber));
            nAnswer = nQuestionIIILibrary.getCorrectAnswer(nQuestionNumber);
            nQuestionNumber++;
        }
    }
    ...
    private void finishQuiz() {
        if (nScore >= 8) {
            Intent intentGif = new Intent(QuizThreeActivity.this, GifActivity.class);
            startActivity(intentGif);
        } else {
            Intent intentLevels = new Intent(QuizThreeActivity.this, LevelsActivity.class);
